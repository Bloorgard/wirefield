...
Каждый жгут получает runtime-массив внутренних точек. Старт и закреплённый хвост выступают ограничениями. Verlet-подобный шаг использует гравитацию, демпфирование и десять проходов коррекции расстояния. Физика идёт с фиксированным шагом 60 Гц через аккумулятор, поэтому поведение не зависит от частоты дисплея; в release-фазе гравитация плавно нарастает от 68% до полной.

Закреплённый жгут со слабиной начинается с параболической заготовки: прямая линия между двумя пинами — неустойчивое равновесие, боковой силы нет, и жгут остаётся натянутым сколь угодно долго вместо провисания. Стрела прогиба берётся как `sqrt(3·пролёт·слабина/8)`, снизу ограничена половиной слабины, сверху половиной длины.
...
`collectSegmentCandidates()` собирает кандидатов один раз на шаг физики (с запасом clearance + клетка), и все проходы solver'а работают по одному списку. Внутренние десять проходов используют позиционную коррекцию без изменения скорости (`response: 'position'`); трение и реституция применяются только в двух финальных settle-проходах.

`slop` — разрешённое проникновение (0.04 клетки): внутри него позиция не правится, иначе лежащий на пине жгут получает поправку каждый кадр и качается через положение. Скорость в сторону пина гасится и внутри допуска, поэтому точка не разгоняется в пин. С допуском сцена в покое замирает полностью.

Нормаль контакта при нулевом расстоянии берётся из прошлого взаимного положения, а не из перпендикуляра к отрезку: у перпендикуляра произвольный знак, и пин, приземлившийся точно на жгут, толкал его против собственного движения. Симметрия здесь массовая — сетка редактора выравнивает жгуты и пины по клеткам.

Замеры на сцене «пин проносится сквозь висящий жгут» показали, что разрешение контактов нужно во **всех** десяти проходах: при контактах через один или только в последнем проходе жгут проходит сквозь пин. `maxPush` ниже 0.5 клетки тоже нельзя — перенос слабеет и дрожание возвращается.

## Движущиеся пины

Пин несёт позицию из предыдущей сборки индекса (`px`, `py`) и попадает в buckets по всему пути движения за кадр, поэтому быстрый пролёт регистрируется независимо от того, где пин остановился. `querySegmentPins()` схлопывает дубликаты по `order`, так что пин из нескольких buckets возвращается один раз.

Solver сравнивает сторону сегмента относительно пина сейчас и на прошлом шаге. Смена знака означает пересечение: нормаль берётся из прошлой стороны, а глубина считается как `clearance + distance`, то есть выталкивание идёт обратно через центр. Без этого жгут под натяжением проскакивал сквозь пин.

Во время drag индекс собирается каждый кадр из runtime-позиций, а не из модели, которая обновляется только на pointerup. Карта прошлых позиций (`pinPrevPositions`) живёт отдельно от кэша индекса и переживает `renderUI()`: иначе перерисовка на pointerup стирала бы историю ровно в кадр самого большого скачка пина. Флаг `swept` держит жгуты разбуженными, пока в сцене есть движущиеся пины.
