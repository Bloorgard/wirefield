...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await reset();
      const collapsed = await evaluate(`(()=>{const side=document.querySelector('.side'),picker=document.querySelector('#mobileWirePicker'),list=document.querySelector('#list'),objects=document.querySelector('.objects'),quick=document.querySelector('#compactLayerControls'),legacy=document.querySelector('#inspector'),controls=document.querySelector('#mobileWireControls');return {height:parseFloat(getComputedStyle(side).height),picker:getComputedStyle(picker).display,list:getComputedStyle(list).display,objects:getComputedStyle(objects).display,controls:getComputedStyle(controls).display,wireName:document.querySelector('#mobileWireName').textContent,add:getComputedStyle(document.querySelector('#addBtn')).display,sort:getComputedStyle(document.querySelector('#sortLayers')).display,clear:getComputedStyle(document.querySelector('#clearBtn')).display,zoomOut:getComputedStyle(document.querySelector('#zoomOut')).display,zoomSlider:getComputedStyle(document.querySelector('#zoom')).display,quick:Boolean(quick),legacy:Boolean(legacy),toolDirection:getComputedStyle(document.querySelector('.tool-controls')).flexDirection,actionDirection:getComputedStyle(document.querySelector('.top-actions')).flexDirection}})()`);
      assert(collapsed.height >= 500 && collapsed.height <= 520, `collapsed panel is ${collapsed.height}px`);
      assert(collapsed.picker !== 'none' && collapsed.controls !== 'none' && collapsed.wireName.startsWith('WIRE_'), 'compact selected-wire controls are hidden');
      assert(collapsed.list === 'none', 'mobile layer list is open by default');
      assert(collapsed.objects === 'none' && collapsed.zoomOut === 'none' && collapsed.zoomSlider === 'none', 'closed panel still exposes list actions or zoom controls');
      assert(collapsed.toolDirection === 'column' && collapsed.actionDirection === 'column', 'mobile top menus are not stacked');
      const expanded = await evaluate(`(()=>{const b=document.querySelector('#mobileWirePicker');b.click();const list=document.querySelector('#list'),side=document.querySelector('.side'),sr=side.getBoundingClientRect(),lr=list.getBoundingClientRect();return {open:side.classList.contains('layers-open'),list:getComputedStyle(list).display,fullWidth:lr.width>sr.width-30}})()`);
